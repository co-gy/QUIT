...
qe_vec_path = f"qe_vec/{args.dataset}-{short_model_name}-qe_vec.npy"
wmt_data_path = f"mt_data/{args.dataset}-genmt-{args.model}-{args.hypo_nums}.jsonl"
print("loading QE vectors...")
qe_vec_lst = np.load(qe_vec_path)
print("loading wmt data...")
with open(wmt_data_path, "r", encoding="utf-8") as f:
    wmt_data = [json.loads(line) for line in f]
sentence_num = qe_vec_lst.shape[0]
hypothesis_num = qe_vec_lst.shape[1]
print("=" * 10, "  data  ", "=" * 10)
print(f"QE vectors({qe_vec_lst.shape}):", qe_vec_path)
...
for sentence_id in range(sentence_num):
    ...
    for i in range(1, hypothesis_num // batch_size + 1):
        ...
        if len(M_window) == window_size and max(M_window) - min(M_window) <= alpha:
            stop_N.append(N)
            candidate_id = qe_vec[:N].argmax()
            ada_qe.append(candidate[candidate_id])
            break

    else:
        stop_N.append(hypothesis_num)
        candidate_id = qe_vec.argmax()
        ada_qe.append(candidate[candidate_id])


# ======================================================================
# --- save result ---
ada_output = f"{txt_dir}/QUIT-qe-{args.hypo_nums}-(k={batch_size}|w={window_size}|alpha={alpha}).txt"
def oneline(s):
    LINE_SEPARATORS = re.compile(r"[\n\r\v\f\x1c-\x1e\x85\u2028\u2029]+")
    return LINE_SEPARATORS.sub(" ", s).strip()
with open(ada_output, "w", encoding="utf-8") as f:
    f.writelines([oneline(sentence)+"\n" for sentence in ada_qe])

# ----- analyze -----
standard_utility_calls = sentence_num * hypothesis_num
ada_utility_calls = sum(stop_N)
print("=" * 10, "analyze", "=" * 10)
print(f"qe model calls ratio: {ada_utility_calls / standard_utility_calls:.4f}")
...
with open(f"{args.dataset}-{short_model_name}-qe-analyze.json", "w") as f:
